...
def parallel_map(
    func: Callable, items: Iterable, max_workers: int = None, show_progress: bool = True
) -> List[Any]:
    """
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_item):
            ...
            if show_progress and (completed % 10 == 0 or completed == total_items):
                ...
                remaining = (
                    (total_items - completed) / items_per_sec
                    if items_per_sec > 0
                    else 0
                )
    ...
    if show_progress:
        ...
        print(
            f"Completed {len(results)}/{total_items} items in {total_time:.2f} seconds "
