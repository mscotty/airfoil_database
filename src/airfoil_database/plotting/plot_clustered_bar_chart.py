...
def plot_clustered_bar_chart_vertical(data, 
                             x_column, 
                             y_column, 
                             output_dir=None, 
                             output_name=None, 
    ...
    """
    @brief Plot a clustered bar chart.
    ...
    # Update layout for clustered bar chart
    fig.update_layout(
        barmode='group',
        title=dict(
            text=title_name if title_name else f"Clustered Bar Chart of {y_column} by {x_column}",
    ...
    if output_dir:
        ...
        file_path = os.path.join(output_dir, output_name if output_name else f"clustered_bar_chart.png")
        fig.write_image(file_path, format="png", width=800, height=600)
    else:
        fig.show()


def plot_clustered_bar_chart_horizontal(data, 
                             y_column, 
                             x_column=None, 
                             output_dir=None, 
                             output_name=None, 
    ...
    fig.update_layout(
        barmode='group',
        title=dict(
            text=title_name if title_name else f"Clustered Bar Chart of Values by {y_column}",
            font=dict(family=title_font_name, size=title_font_size),
            x=0.5  # Center title
        ),
        xaxis=dict(
