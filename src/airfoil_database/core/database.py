...
class AirfoilDatabase:
    ...
    def store_airfoil_data(
        self, name, description, pointcloud, airfoil_series, source, overwrite=False
    ):
        try:
            with Session(self.engine) as session:
                ...
                if existing_airfoil and not overwrite:
                    print(
                        f"Airfoil {name} already exists in the database. Use overwrite=True to update."
                    )
                    return
    ...
    def store_bulk_airfoil_data(self, data_list, overwrite=False):
        ...
        inserted_count = 0
        names_to_check = {
            data["name"] for data in data_list if "name" in data
        }  # Get unique names from input
            ...
            with Session(self.engine) as session:
                if not overwrite:
                    ...
                    statement = select(Airfoil.name).where(
                        Airfoil.name.in_(names_to_check)
                    )
                    existing_names = {row.name for row in session.exec(statement)}
                    logging.info(
                        f"Found {len(existing_names)} existing airfoils matching input names. Skipping them."
                    )
                else:
                ...
                for data in data_list:
                    ...
                    if overwrite:
                        ...
                        if existing_airfoil:
                            ...
                            existing_airfoil.airfoil_series = data.get(
                                "airfoil_series", "UNKNOWN"
                            )
                ...
                logging.info(
                    f"Bulk insert/replace finished. Affected rows: {inserted_count}"
                )
        ...
        with Session(self.engine) as session:
            ...
            for airfoil in airfoils:
                ...
                if airfoil_series_curr == AirfoilSeries.OTHER:
                    airfoil_series_curr = AirfoilSeries.identify_airfoil_series(
                        airfoil.name
                    )
                    if airfoil_series_curr == AirfoilSeries.OTHER:
                        airfoil_series_curr = AirfoilSeries.identify_airfoil_series(
                            airfoil.description or ""
                        )
    ...
    def get_airfoil_dataframe(self):
        ...
        for name, series, pointcloud in results:
            ...
            data.append({"Name": name, "Series": series, "Num_Points": num_points})
    ...
    def store_aero_coeffs(self, name, reynolds_number, mach, ncrit, alpha, cl, cd, cm):
        ...
        try:
            with Session(self.engine) as session:
                ...
                session.commit()
                logging.debug(
                    f"Stored coeffs for {name}: Re={reynolds_number}, M={mach}, α={alpha}, Cl={cl}, Cd={cd}, Cm={cm}"
                )
        except Exception as e:
            logging.error(
                f"Error storing aero coeffs for {name} Re={reynolds_number} M={mach} A={alpha}: {e}"
            )
        ...
        try:
            with Session(self.engine) as session:
                ...
                for airfoil_name in airfoil_names:
                    # Check if airfoil exists
                    airfoil_statement = select(Airfoil).where(
                    ...
                    for row in session.exec(existing_statement).all():
                        existing_conditions.add(
                            (row.reynolds_number, row.mach, row.alpha, row.ncrit)
                        )
                    ...
                    if airfoil_missing:
                        ...
                        logging.info(
                            f"Found {len(airfoil_missing)} missing conditions for {airfoil_name}"
                        )
                    else:
    ...
    def get_airfoil_geometry_dataframe(self):
        ...
        try:
            with Session(self.engine) as session:
                ...
                # Convert SQLModel objects to dictionaries with all new fields
                data = []
                for geom in results:
                    data.append(
                        {
                            ...
                            "trailing_edge_angle": geom.trailing_edge_angle,
                            "thickness_to_chord_ratio": geom.thickness_to_chord_ratio,
                            "max_thickness_position": geom.max_thickness_position,
                            "max_camber_position": geom.max_camber_position,
                            "thickness_distribution": geom.thickness_distribution,
                            "camber_distribution": geom.camber_distribution,
                            "normalized_chord": geom.normalized_chord,
                        }
                    )
            ...
            logging.error(f"Error retrieving geometry dataframe: {e}")
            return pd.DataFrame()  # Return empty dataframe on error

    def get_airfoils_with_geometry(self):
        """
        Returns a list of airfoil names that have geometry data.

        Returns:
            list: List of airfoil names with geometry data
        """
        try:
            with Session(self.engine) as session:
            ...
            logging.error(f"Error getting airfoils with geometry: {e}")
            return []
